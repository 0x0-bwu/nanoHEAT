                  ...
                  const PrismMeshSettings & meshSettings, typename PrismThermalModel::PrismTemplate & triangulation)
{
    if (meshSettings.dumpMeshFile) {
        GeometryIO::WritePNG(std::string(nano::CurrentDir()) + "/meshIn.png", polygons.begin(), polygons.end(), 4096);
        GeometryIO::WriteWKT<NPolygon>(std::string(nano::CurrentDir()) + "/meshIn.wkt", polygons.begin(), polygons.end());
    }
